...
def make_sub_problem(subp, N, n):
    ...
    def sub_problem(x_init, y, mu):
        ...
        optimizer = IPOPT(prob, solver_options={'max_iter': 500, 'tol': 1e-10}, turn_off_outputs=True)
        results = optimizer.solve()
        optimizer.print_results()

        objective.append(results['f'])
        time.append(results['time'] + (time[-1] if len(time)>0 else 0))
