...
if __name__ == "__main__":
    ...
    optimizer = SteepestDescent(prob, maxiter=3000, opt_tol=1e-3, turn_off_outputs=True)

    results = optimizer.solve()
    optimizer.print_results()


    # ===== Plot Rosenbrock contour with iterates =====
    iterates = results['iterates']
    ...
    plt.contour(X, Y, Z, levels=np.logspace(-0.5, 3.5, 35), cmap='jet')
    plt.plot(iterates[:, 0], iterates[:, 1], 'o-', color='red', markersize=4, linewidth=1.5)
