...
X, Y = np.meshgrid(x, y)
Z = (1 - X)**2 + 5 * (Y - X**2)**2
plt.contour(X, Y, Z, levels=np.linspace(0, 50, 20), cmap='Blues_r', alpha=0.4, linewidths=0.5)
plt.contourf(X, Y, Z, levels=np.linspace(0, 50, 20), cmap='Blues_r', alpha=0.5)

plt.plot(x1_history, x2_history, '-', color='tab:red', linewidth=2, markersize=4, zorder=10)
plt.xlim(-1.5, 1.5)
plt.ylim(-1.5, 1.5)
plt.xlabel('x')
plt.ylabel('y')
