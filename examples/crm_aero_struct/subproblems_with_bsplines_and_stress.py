...
ns = 65#45 # num spanwise panels (must be odd)
...
def make_subproblem(subP, num, opt_time, samples):
    ...
    def subP_i(v_init: list,
        ...
        def objective(x):
            ...
            drag_coefs = []
            for i in range(num):
                # twist_cp_i = twist_cps[i]
                twist_cp_i = twist_cps[0]
                twist_i = bspline_comp(twist_bspline_mtx, twist_cp_i)
            ...
            # obj += jnp.sum(jnp.array(alphas)**2) * 1e1 # remove the differential flatness in the trim solution
        ...
        if subP == 0:
            alpha_lower = -1 * np.ones(1) * np.deg2rad(0)
            alpha_upper = np.ones(1) * np.deg2rad(0)
        else:
