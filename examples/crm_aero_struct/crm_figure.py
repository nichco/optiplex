...
# crm = pv.read("examples/crm_aero_struct/uCRM_9_wbt.stl")
crm = pv.read("examples/crm_aero_struct/crm_no_wing.stl")
# scale the crm mesh
crm = crm.scale(25.4, inplace=True)

# plotter.add_mesh(plane, color="lightgray", show_edges=False, opacity=1)
# plotter.add_mesh(crm, color="white", show_edges=False, opacity=1)
plotter.add_mesh(crm, color="lightgray", show_edges=False, opacity=1, silhouette=False)
...
plotter.add_mesh(mesh, cmap="plasma", show_edges=True, lighting=True, smooth_shading=True, show_scalar_bar=False,)
...
plotter.view_vector(vector=[-1.5, -1, 1.5]) 
plotter.show()
# plotter.screenshot('crm.png', transparent_background=True, window_size=[3000, 2000])